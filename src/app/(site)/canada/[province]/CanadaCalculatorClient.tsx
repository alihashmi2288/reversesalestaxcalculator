...
import type { ProvinceRate } from '@/data/canadaTaxRates';

interface Props { provinceData: ProvinceRate; }

export default function CanadaCalculatorClient({ provinceData }: Props) {
  const [finalPrice, setFinalPrice] = useState('');
  const [taxRate, setTaxRate] = useState(String(provinceData.rate));
  ...
  const reset = () => { setFinalPrice(''); setTaxRate(String(provinceData.rate)); setResult(null); setError(''); };

  const copy = () => {
    if (!result) return;
    navigator.clipboard.writeText(`Canada ${provinceData.name} Tax Calculation\nTotal: $${fmt(result.totalPaid)}\nRate: ${result.taxRate}%\nPre-Tax: $${fmt(result.preTaxPrice)}\nTax: $${fmt(result.taxAmount)}`);
    setCopied(true); setTimeout(() => setCopied(false), 2000);
  };

  return (
    <div className="calc-card" style={{ border: '2px solid var(--primary)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }} className="grid-responsive">
        <div>
          <label htmlFor="ca-prov-final-price" style={{ display: 'block', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8, color: 'var(--text-primary)' }}>
            Total Price Paid (CAD, with tax)
          </label>
          <div className="input-wrapper">
            <span className="input-prefix" aria-hidden="true">$</span>
            <input
              id="ca-prov-final-price"
              ...
              step="0.01"
              placeholder="113.00"
              aria-label={`Total Price Paid in ${provinceData.name} in CAD with tax`}
              value={finalPrice}
        ...
        <div>
          <label htmlFor="ca-prov-tax-rate" style={{ display: 'block', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8, color: 'var(--text-primary)' }}>
            {provinceData.name} Tax Rate
          </label>
          <div className="input-wrapper">
            <input
              id="ca-prov-tax-rate"
              ...
              min="0"
              step="0.001"
              aria-label={`${provinceData.name} tax rate percentage`}
              value={taxRate}
          ...
          <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>
            Pre-filled with {provinceData.name}&apos;s {provinceData.type} rate
          </p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        {provinceData.gst > 0 && <span style={{ background: 'rgba(204, 255, 0, 0.08)', border: '1px solid rgba(204, 255, 0, 0.3)', color: 'var(--primary)', padding: '4px 10px', borderRadius: 6, fontSize: 13, fontWeight: 600 }}>GST: {provinceData.gst}%</span>}
        {provinceData.pst > 0 && <span style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', color: 'var(--text-secondary)', padding: '4px 10px', borderRadius: 6, fontSize: 13, fontWeight: 600 }}>PST: {provinceData.pst}%</span>}
        {provinceData.hst > 0 && <span style={{ background: 'rgba(204, 255, 0, 0.08)', border: '1px solid rgba(204, 255, 0, 0.3)', color: 'var(--primary)', padding: '4px 10px', borderRadius: 6, fontSize: 13, fontWeight: 600 }}>HST: {provinceData.hst}%</span>}
        <span style={{ background: 'var(--primary)', color: '#000', padding: '4px 10px', borderRadius: 6, fontSize: 13, fontWeight: 800 }}>Combined: {provinceData.rate}%</span>
      </div>
      ...
      <button suppressHydrationWarning onClick={calculate} className="btn-primary" style={{ minHeight: 56, marginBottom: 24 }}>
        ...
        Calculate {provinceData.name} Pre-Tax Price
      </button>
        ...
        <div style={{ animation: 'slideIn 0.3s ease' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }} className="grid-results">
            ...
            <div className="result-card success" style={{ background: 'var(--card-bg)' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Excluding {provinceData.type}</div>
            </div>
            ...
            <div className="result-card danger" style={{ background: 'var(--card-bg)' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Total GST/HST/PST paid</div>
            </div>
            ...
            <div className="result-card neutral" style={{ background: 'var(--card-bg)' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Checkout inclusive total</div>
            </div>
          </div>
