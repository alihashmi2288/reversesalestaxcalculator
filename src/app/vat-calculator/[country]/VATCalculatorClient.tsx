...
import type { CountryVatRate } from '@/data/vatTaxRates';

interface Props { countryData: CountryVatRate; }

export default function VATCalculatorClient({ countryData }: Props) {
  const [finalPrice, setFinalPrice] = useState('');
  const [taxRate, setTaxRate] = useState(String(countryData.rate));
  ...
  const reset = () => { setFinalPrice(''); setTaxRate(String(countryData.rate)); setResult(null); setError(''); };

  const copy = () => {
    if (!result) return;
    navigator.clipboard.writeText(`${countryData.name} VAT Calculation\nTotal: ${countryData.symbol}${fmt(result.totalPaid)}\nRate: ${result.taxRate}%\nEx-VAT: ${countryData.symbol}${fmt(result.preTaxPrice)}\nVAT Amount: ${countryData.symbol}${fmt(result.taxAmount)}`);
  ...
  return (
    <div className="calc-card" style={{ border: '2px solid var(--primary)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }} className="grid-responsive">
        <div>
          <label htmlFor="vat-calc-final-price" style={{ display: 'block', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8, color: 'var(--text-primary)' }}>
            Total Price Paid ({countryData.currency}, with tax)
          </label>
          <div className="input-wrapper">
            <span className="input-prefix" aria-hidden="true">{countryData.symbol}</span>
            <input
              id="vat-calc-final-price"
              ...
              step="0.01"
              placeholder="120.00"
              aria-label={`Total Price Paid in ${countryData.name} in ${countryData.currency} with tax`}
              value={finalPrice}
        ...
        <div>
          <label htmlFor="vat-calc-tax-rate" style={{ display: 'block', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8, color: 'var(--text-primary)' }}>
            {countryData.name} {countryData.type} Rate
          </label>
          <div className="input-wrapper">
            <input
              id="vat-calc-tax-rate"
              suppressHydrationWarning
              type="number"
              inputMode="decimal"
              min="0"
              step="0.001"
              aria-label={`${countryData.name} ${countryData.type} rate percentage`}
              value={taxRate}
          ...
          <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>
            Pre-filled with {countryData.name}&apos;s standard {countryData.type} rate
          </p>
        </div>
      </div>
      ...
      <button suppressHydrationWarning onClick={calculate} className="btn-primary" style={{ minHeight: 56, marginBottom: 24 }}>
        ...
        Calculate {countryData.name} Net Price
      </button>
      ...
      {result && (
        <div style={{ animation: 'slideIn 0.3s ease' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }} className="grid-results">
            ...
            <div className="result-card success" style={{ background: 'var(--card-bg)' }}>
              <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--primary)', marginBottom: 8 }}>
                ✓ Net Price (Ex-{countryData.type})
              </div>
              <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--primary)', lineHeight: 1 }}>
                {countryData.symbol}{fmt(result.preTaxPrice)}
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Excluding {countryData.type}</div>
            </div>
            ...
            <div className="result-card danger" style={{ background: 'var(--card-bg)' }}>
              <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#f87171', marginBottom: 8 }}>
                {countryData.type} Amount
              </div>
              <div style={{ fontSize: 36, fontWeight: 800, color: '#ef4444', lineHeight: 1 }}>
                {countryData.symbol}{fmt(result.taxAmount)}
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>{result.taxRate}% of net amount</div>
            </div>
            ...
            <div className="result-card neutral" style={{ background: 'var(--card-bg)' }}>
              ...
              <div style={{ fontSize: 36, fontWeight: 800, color: '#ffffff', lineHeight: 1 }}>
                {countryData.symbol}{fmt(result.totalPaid)}
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Tax-inclusive total</div>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--bg)', border: '2px dashed var(--border)', borderRadius: 'var(--radius)', padding: '12px 16px', fontSize: 13, color: 'var(--primary)', marginTop: 16, fontFamily: 'var(--font-mono)', wordBreak: 'break-word' }}>
            <strong>Calculation formula:</strong> {countryData.symbol}{fmt(result.preTaxPrice)} = {countryData.symbol}{fmt(result.totalPaid)} ÷ (1 + {result.taxRate}/100)
          </div>
        </div>
      )}
